...
class line_planning_solver:
	...
	def solve_master_LP(self):
		...
		#max_frequency = nb_pass // capacity + 1
		...
		#TODO define the line_costs as part of the instance
		#lines_cost = [fixed_cost + l % max_frequency for l in range(nb_lines * max_frequency)]
		
		#cost proportional to line length
		#lines_cost = [fixed_cost * self.line_instance.set_of_lines[l//max_frequency][0] + l % max_frequency * self.line_instance.set_of_lines[l//max_frequency][0] for l in range(nb_lines * max_frequency)]
		...
		#line_cost = self.line_instance.lines_cost
		...
		t_2 = time.time()
		#print("initialization finsihed", t_2-t_1)
		...
		while t_4 - t_2 <= allowed_time:
			...
			#print('time column_generation iteration:', t_3 - t_0)
		...
		for l in range(len(lines_to_sets_temp)):
			...
			for s in lines_to_sets_temp[l]:
				if x_temp[l,s]>0:
					...
					if not l_activated:
						budget += lines_cost[l]
					#print("line", l, s, x[l,s].X)
					#print('set', sets[s])
					#print('lines_cost', lines_cost[l])
					l_activated = True
	...
	def solve_ILP(self, method = 0):
		...
		nb_pass = self.line_instance.nb_pass
		nb_lines = self.line_instance.nb_lines
		print('nb_pass=', nb_pass, 'nb_lines=', nb_lines * max_frequency)
		capacity = self.line_instance.capacity
		#max_frequency = nb_pass // capacity + 1
		t_1 = time.time()

		
		#lines_cost = [fixed_cost + l % max_frequency for l in range(nb_lines * max_frequency)] # a modifier
		...
		master = Model("ILP") # master LP problem
		master.ModelSense = -1

		master.Params.timeLimit = allowed_time
		#master.setParam( 'OutputFlag', False )

		#print(self.line_instance.values)
		...
		var = [y[l] for l in range(len(lines_to_sets))]
		coef = [lines_cost[l] for l in range(len(lines_to_sets))]
	...
	def solve_ILP(self, method = 0):
		...
		passengers_to_sets = [[[] for l in range(nb_lines * max_frequency)] for p in range(nb_pass)] #for passenger index i, contains an array whose first element is  	
		
		# generate initial sets with one passenger covered per line
		for l in range(len(lines_to_sets)):
			route = l // max_frequency
			if len(self.line_instance.set_of_lines[route][1]) != 0:
				p = self.line_instance.set_of_lines[route][1][0][0]
		...
		#print('sets', sets)
		#print('passengers_to_sets', passengers_to_sets)
		#print("lines", lines_to_passengers)
		#print("pass", passengers_to_sets)
		...
		'''master.set(GRB.IntParam.PreCrush, 1);
		master.set(GRB.IntParam.Presolve, 0);
		master.set(GRB.IntParam.Cuts, 0);
		master.set(GRB.IntParam.NoRelHeuristic, 0); '''
		master.Params.Presolve =0
		master.Params.PreCrush = 1
		master.Params.Cuts = 0
		master.Params.NoRelHeuristic = 0
...
if __name__ == "__main__":
	...
	#nb_p = 1
	Budget = 0
	line_inst = line_instance(nb_lines = nb_l, nb_pass = nb_p, B = 0.95 * Budget, cost=1, max_length = 15, min_length = 8, proba = 0.1, capacity = 30, instance_category = 'manhattan', detour_factor = 3, method = 3, granularity = 1)
	...
	for ind in range(0,5):
		...
		while iter <= 10000:
		    #used_budget, value, opened_lines = solver.rounding(solution, active_sets, sets)
		...
		#print('values', solver.line_instance.values)
		if nb_respect > 0 :
			#print("mean", mean/nb_respect, nb_respect)
			print('opened', op)
			...
			#print('pass_ass', passenger_assignment)
		...
		try: 
			...
			average_value_ILP += obj_val
			time_ILP += run_time_ILP
		except:
